...
# DEBUG Job信息
print("DEBUG Job信息: {}".format(server.debug_job_info('test')))
'''
...
# 创建Job
try:
    server.create_job('my-github', jenkins.RECONFIG_XML)
except :
    print("项目已经存在")
...
# 触发Job(方式一)
params=None
server.build_job_url('test', parameters=params)
'''
...
server.set_next_build_number('test', next_bn + 5)

# 指定编号的构建Job信息
print(server.get_build_info('test', 48))
'''
